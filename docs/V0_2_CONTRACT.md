...
main brief는 다음 10개 섹션을 정확한 순서로 포함한다.
...
1. 긴 서론 없이 적용할 변경부터 제시한다.
2. 첫 8개 비어 있지 않은 줄 안에 대상 파일과 변경 목적을 표시한다.
3. 제공된 코드와 공개 맥락만 근거로 patch 또는 교체 코드를 작성한다.
4. 숨긴 프로젝트 구조, 필드, 함수와 호출 방식을 추측하지 않는다.
5. 변경 동작을 검증하는 테스트를 함께 제시한다.
6. 실제 실행하지 않은 테스트를 실행했다고 표현하지 않는다.
7. 추가 확인이 필요하면 최대 2개만 적는다.
8. 외부 API 동작을 주장할 때 가능한 경우 버전이 고정된 공식 문서를 사용한다.
9. 작업 요청이 별도로 요구하지 않으면 비어 있지 않은 줄 80개 이내로 답한다.
