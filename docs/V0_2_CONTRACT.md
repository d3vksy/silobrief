...
- 문서 상태: `RELEASED WITH CLAUDE-ONLY VALIDATION`
- 제품 버전: `0.2.0`
- Markdown 계약 버전: `2`
...
2026-08-05 출시 결정에서 Claude는 revision 3 과제 3개를 모두 통과했지만 GPT는
실행하지 않았다. 프로젝트 소유자는 이 제한을 공개적으로 기록하는 조건으로 v0.2.0
출시를 승인하고 GPT 검증을 후속으로 미뤘다. 따라서 양 모델 검증 완료를 주장하지 않는다.
...
2. 대상 파일과 변경 목적을 먼저 적고, 공개된 코드와 맥락만 근거로 diff-style 변경
   코드를 작성한다.
...
- 하나 이상의 `diff` fenced block을 사용한다.
- 수정 작업은 변경 전 줄을 `-`, 변경 후 줄을 `+`로 표시한다.
- 변경하지 않은 문맥 줄은 필요한 만큼 함께 표시할 수 있다.
- 파일 header, hunk header와 정확한 줄 번호는 선택 사항이다.
- machine-applicable patch 또는 `git apply` 성공을 요구하거나 주장하지 않는다.
- 전체 교체용 일반 코드 블록으로 대신하지 않는다.
...
- 숨긴 구현을 사실처럼 추측하지 않음
- 읽기 쉬운 `diff` fenced block이 있고 수정 전 `-`, 수정 후 `+` 표기가 있음
- 동작 중심 테스트가 있음
...
출시 시점의 예외 결정은 `validation/v0.2/results/CLAUDE_GATE_RESULT.md`에 기록한다.
Claude는 3개 중 3개를 통과했으며 GPT는 실행하지 않았다. 이는 원래 양 모델 게이트를
충족한 것으로 간주하지 않고, 소유자가 승인한 Claude-only MVP 출시로만 표현한다.
